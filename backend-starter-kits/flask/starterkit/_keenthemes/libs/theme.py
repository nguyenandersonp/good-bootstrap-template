...
class KTTheme:
    ...
    def getIcon(name, class_name='', type=''):
        ...
        if type == 'duotone':
            icons = current_app.config.get('duotone-icons')
            if not icons:
                with open(os.path.dirname(os.path.abspath(__file__)) + '/icons.json') as f:
                    icons = json.load(f)
                current_app.config['duotone-icons'] = icons
